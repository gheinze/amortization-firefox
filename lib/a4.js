var a4 = (function () {
    ...
    var _nextDate = function(startDate, paymentFrequency, steps) {
        ...
        switch (paymentFrequency) {
            ...
            case 1:  return moment(startDate).add(1 * steps, 'years').toDate();
            case 2:  return moment(startDate).add(6 * steps, 'months').toDate();
            case 4:  return moment(startDate).add(3 * steps, 'months').toDate();
            case 6:  return moment(startDate).add(2 * steps, 'months').toDate();
            case 12: return moment(startDate).add(1 * steps, 'months').toDate();
            case 26: return moment(startDate).add(2 * steps, 'weeks').toDate();
            case 52: return moment(startDate).add(1 * steps, 'weeks').toDate();
            ...
            case 24:
                ...
                if (steps %2 ==0) {
                    return moment(modifiedStartDate).add(steps / 2, 'months').toDate();
                }

                let prevDate = moment(modifiedStartDate).add( (steps - 1) / 2, 'months');
                if (modifiedStartDate.date() < 15) {
                    return prevDate.add(14, 'days').toDate();
                }

                let deltaDays = modifiedStartDate.date() - 14;

                return moment().add(deltaDays, 'days').toDate();


            default: console.log("Unsupported payment frequency: " + paymentFrequency);
        }
    ...
    */
    var _getInterestOnlyPayments = function(amAttrs) {
        ...
        var totalPayments = amAttrs.termInMonths / (amAttrs.paymentFrequency / 12);
        var paymentList = [];
